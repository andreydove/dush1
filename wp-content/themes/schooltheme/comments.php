...
<div id="comments"> <?php // див с этим id нужен для якорьных ссылок на комменты ?>
	...
	<?php if (comments_open()) { // если комментирование включено для данного поста
		...
		$fields =  array( // разметка текстовых полей формы
			'author' => '<label for="author">Ім\'я<input id="author" name="author" type="text" value="'.esc_attr($commenter['comment_author']).'" size="30" required></label>', // поле Имя
		...
		$args = array( // опции формы комментирования
			...
			'comment_field' => '<label for="comment">Коментар: <textarea id="comment" name="comment" cols="45" rows="8"></textarea></label>', // разметка поля для комментирования
			'must_log_in' => '<p class="must-log-in">Вам необхідно зареєструватись '.wp_login_url(apply_filters('the_permalink',get_permalink())).'</p>', // текст "Вы должны быть зарегистрированы!"
			'logged_in_as' => '<p class="logged-in-as">'.sprintf(__( 'Ви увійшли як <a href="%1$s">%2$s</a>. <a href="%3$s">Вийти?</a>'), admin_url('profile.php'), $user_identity, wp_logout_url(apply_filters('the_permalink',get_permalink()))).'</p>', // разметка "Вы вошли как"
			'comment_notes_before' => '<p class="comment-notes">Ваш email не буде опубліковано.</p>', // Текст до формы
			'comment_notes_after' => '<p class="form-allowed-tags">'.sprintf(__( 'Ви можете використати наступні <abbr>HTML</abbr> тtги: %s'),'<code>'.allowed_tags().'</code>').'</p>', // текст после формы
